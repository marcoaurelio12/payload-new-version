...
#### Buscar Proposta por Slug (PÚBLICO)

```typescript
async function getProposalBySlug(slug: string) {
  const response = await fetch(
    `${payloadUrl}/api/proposals/public/${slug}`
  )
...
// Uso:
const proposal = await getProposalBySlug('o_select')
console.log(proposal.title, proposal.client.name, proposal.pricing)
```
...
async function getProposalByToken(token: string) {
  const response = await fetch(
    `${payloadUrl}/api/proposals/public/token/${token}`
  )
...
// Uso:
const proposal = await getProposalByToken('uuid-token-from-email')
```

> **Nota:** Estes endpoints públicos atualizam automaticamente o status da proposta para "viewed" quando acedidos pela primeira vez.
